...
namespace tor_tools
{
    public partial class ModelBrowser : Form
    {
        ...
        private async void TreeViewList_AfterSelect(object sender, TreeViewEventArgs e)
        {
            TreeNode selectedNode = treeViewList.SelectedNode;
            NodeAsset tag = (NodeAsset)selectedNode.Tag;
            this.Text = "Model Browser - " + tag.Id.ToString();
                ...
                if (file != null)
                {
                    ...
                    if (gr2_model.materials.Count == 0)
                    {
                        ...
                        if (gr2_model.numMaterials == 1)
                            ...
                            {
                                new GR2_Material("default")
                            };

                        if (gr2_model.numMaterials == 2)
                            gr2_model.materialOverride = new GR2_Material("defaultMirror");
                    }
                    // else
                    // {
                    //     foreach (GR2_Material mat in gr2_model.materials)
                    //     {
                    //         if (mat.materialName == "default")
                    //             mat.materialName = "disco_ball_mat";
                    //     }
                    // }
...
#pragma warning disable CS1998, CS4014
        ...
        private async Task PreviewMNT_GR2(GomObjectData obj)
        {
            ...
            else
            {
                ...
                obj.Dictionary.TryGetValue("4611686299207604004", out object npcNodeId);
                if (npcNodeId != null)
                {
...
#pragma warning restore CS1998, CS4014
        ...
        private void ParseNpcData(NpcAppearance npcData)
        {
            ...
            {
                string bodyType;
                bodyType = slotDict.Value[0].BodyType;
                string model;
                model = slotDict.Value[0].Model.Replace("[bt]", bodyType);
                if (model.Contains(".gr2"))
                {
                    ...
                    if (filemodel != null)
                    {
                        ...
                        string name = model.Split('/').Last();
                        FileFormats.GR2 gr2_model = new FileFormats.GR2(br, name);
                        string mat0 = slotDict.Value[0].Material0.Replace("[bt]", bodyType);
                        string matMir = slotDict.Value[0].MaterialMirror.Replace("[bt]", bodyType);
                        string palette1XML = "";
                        string palette2XML = "";

                        // HACK: Until I can figure out why this isn't being read properly in the first place!
                        if (mat0.Contains("head_bloodsith_"))
                            matMir = "eye_sith_non_a01_c01";
                        if (mat0.Contains("head_bmn_torian_") | mat0.Contains("head_human_corso_") | mat0.Contains("head_human_doc_") | mat0.Contains("head_human_lresso_"))
                            matMir = "eye_human_non_a01_c01";
                        if (mat0.Contains("head_human_bmn_vector_"))
                            matMir = "eye_joiner_non_a01_c01";
                        if (mat0.Contains("head_twilek_"))
                            matMir = "eye_human_non_a01_c01";
                        ...
                        if (mat0 != null)
                        {
                            ...
                            {
                                mat0 = mat0.Replace("[gen]", "f");
                                matMir = matMir.Replace("[gen]", "f");
                            }
                            else if (bodyType.Contains("bm"))
                            {
                                mat0 = mat0.Replace("[gen]", "m");
                                matMir = matMir.Replace("[gen]", "m");
                            }

                            gr2_model.materials = new List<GR2_Material>
                                {
                                    new GR2_Material(mat0)
                                };

                            if (matMir != mat0 && matMir != "")
                            //     gr2_model.materialOverride = new GR2_Material(matMir);
                            {
                                if (slotDict.Value[0].Type == "creature")
                                {
                                ...
                                if (resourceName.InnerText.Contains(".gr2"))
                                {
                                    ...
                                    if (attachModel != null)
                                    {
                                        ...
                                        */
                                        ...
                                        {
                                            modelEnabled.TryGetValue(resourceFxName.InnerText, out bool enabled);
            ...
            if (e.Button == MouseButtons.Right)
            {
                ...
                if (treeViewList.SelectedNode != null && treeViewList.SelectedNode.Nodes.Count > 0)
                {
                    ...
                    else if (treeViewList.SelectedNode.Name == "ipp")
                    {
                        ...
                        BodyTypeStrip.Show(treeViewList, e.Location);
                    }
                }
            }
        }
