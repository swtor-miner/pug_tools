...
float4 PS_Skin( VS_OUTPUT pin, uniform int gLightCount ) : SV_Target
{
	...
	float3 HSL = float3(paletteMapValue.g, paletteMapValue.b, paletteMapValue.a);

	// ManipulateHSL Function ---------------------------------------------------------------------
	HSL = ExpandHSL(HSL);

	// Adjust Lightnees
	HSL.b = pow(HSL.b, gPalette1.w) * (gPalette1.w * 1.25); //.w = contrast
	HSL.b = (gPalette1.z + ((1 - gPalette1.z) * HSL.b)) * 0.9; //.z = brightness

	// Offset Hue
	HSL.r = frac(HSL.r + gPalette1.x) - 0.07;

	// Offset Saturation
	HSL.g = pow(HSL.g, gPalette1.y);
	HSL.g = HSL.g * (1 - gPalette1.y);
	HSL.g = saturate(HSL.g + 0.025);

	// End Function -------------------------------------------------------------------------------

	float ambientOcclusion = ManipulateAO(paletteMapValue.r, gPalette1.z, gPalette1.w);
	HSL.z *= ambientOcclusion;

	float3 RGB = ConvertHSLToRGB(HSL);

	float3 diffuseColor = lerp(diffuseMapValue.rgb, RGB.rgb, (paletteMaskSum));
	...
	float3 finalColor = fragmentDiffuseColor.rgb;
