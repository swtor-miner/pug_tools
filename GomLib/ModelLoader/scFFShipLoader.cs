...
namespace GomLib.ModelLoader
{
    public class SCFFShipLoader
    {
        ...
        public ScFFShip Load(ScFFShip shp, long Id, GomObjectData obj)
        {
            ...
            shp.SetMinorComponentsContainerId(obj.Get<ulong>("scFFMinorComponentsPackage")); //ex. 16140923285529548575 (Node conSpec_scff_equip_min_ACRS) 

            shp.SetMinorComponentSlots(new Dictionary<string, long>());
            GomObject minorComponentData = _dom.GetObject(shp.GetMinorComponentsContainerId());
            ...
            shp.SetMinorEquipType(minorEquipType.Replace("conSpec_scff_equip_min_", ""));
            ...
            shp.SetMajorComponentsContainerId(obj.Get<ulong>("scFFMajorComponentsPackage")); //ex. 16141040006346149027 (Node conSpec_scff_equip_maj_PSYHE)

            shp.MajorComponentSlots = new Dictionary<string, long>();
            GomObject majorEquipData = _dom.GetObject(shp.GetMajorComponentsContainerId());
            ...
            shp.SetMajorEquipType(majorEquipType.Replace("conSpec_scff_equip_maj_", ""));
            ...
            shp.SetDamagedPackageNodeId(obj.Get<ulong>("scFFDamagePackage")); //ex. 16141171526721883186 (Node imp_striker_c_damage_package)
            ...
            {
                defaultLoadoutData.TryGetValue((ulong)shp.Id, out object shipLoadoutMap);
            ...
            shp.SetUnknownStat1(obj.ValueOrDefault<float>("4611686298643904002", 0)); //scale ?? ex. 0.15 - Override Power_Shield_Regen_Rate Modifier?
            shp.SetUnknownStat2(obj.ValueOrDefault<float>("4611686298656584000", 0)); // ?? ex. 0.0025

            shp.SetUnknownStat3(obj.ValueOrDefault<float>("4611686348394117006", 0)); //ex. 2
            shp.SetUnknownStat4(obj.ValueOrDefault<float>("4611686348594427000", 0)); //ex. 2
            shp.SetUnknownStat5(obj.ValueOrDefault<float>("4611686348594427001", 0)); //ex. 1.25

            shp.SetUnknownStat6(obj.ValueOrDefault<float>("4611686348976567004", 0)); //ex. 0.032
            shp.SetUnknownStat7(obj.ValueOrDefault<float>("4611686348976567005", 0)); //ex. 0.3
            shp.SetUnknownStat8(obj.ValueOrDefault<float>("4611686348976567007", 0)); //ex. 0.6
            ...
            shp.SetEngStatsNodeId(obj.Get<ulong>("scFFEngineStatsPackage")); //ex. 16140967054983465763 (Node spvp.eng.striker)
            GomObject engStatPackage = _dom.GetObject(shp.GetEngStatsNodeId());
            ...
            if (apcLookup != 0)
            {
                shp.SetAbilityPackage(_dom.abilityPackageLoader.Load(apcLookup)); //ex. 16141125605689795242 (Node apc.spvp.gunship.weapon_swap_secondary)
            }
            return shp;
        }
