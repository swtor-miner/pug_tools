...
namespace GomLib.ModelLoader
{
    public class AbilityLoader : IModelLoader
    {
        ...
        private void LoadParamEffects(List<object> abilityEffectList) //Need to make this output xml
        {
            System.IO.StreamWriter file3 = new System.IO.StreamWriter("c:\\swtor\\abilityeffectgom.txt", true);
            foreach (ulong id in abilityEffectList)
            {
                var obj = _dom.GetObject(id);
                obj.Print(file3);
                obj.Unload();
            }
            file3.Close();
            /*if (abilityEffectList == null) { return ""; }

            var tokenList = new List<string>();

            tokenList.Add("<?xml version=\"1.0\" encoding=\"utf-8\"?>");
            tokenList.Add("<Effects>");
            foreach (ulong effId in abilityEffectList)
            {
                var eff = _dom.GetObject(effId);
                int i = 0;
                ...
                if (eff == null) { break; }
                while (i < eff.Data.Dictionary.Count()) //add check for null
                {
                    string k = eff.Data.Dictionary.ElementAt(i).Key.ToString();
                    //tokenList.Add(String.Format("{0}: {1}", k, eff.Data.ValueOrDefault<object>(k, null).ToString()) + ", ");

                    var value = eff.Data.ValueOrDefault<object>(k, null).ToString();

                    if (k == "effSubEffects")
                    {
                        iteratorThroughLists(eff.Data.Get<List<object>>(k), tokenList, "effSubEffects").ToString();
                        var subEffects = eff.Data.Get<List<object>>(k);
                        int ii = 0;
                        tokenList.Add(String.Format("<effSubEffect_{0}>", ii));
                        while (ii < subEffects.Count()) //add check for null
                        {
                            var subEff = (GomObjectData)subEffects.ElementAt(ii);
                            int iii = 0;
                            while (iii < subEff.Dictionary.Count()) //add check for null
                            {
                                var key = (subEff.Dictionary.ElementAt(iii)).Key.ToString();
                                var subEffEntry = subEff.Get<object>(key.ToString());
                                tokenList.Add(String.Format("<{0} value=\"{1}\" />", key.ToString(), subEffEntry.ToString() + "  "));
                                iii++;
                            }
                    ...
                    {
                        tokenList.Add(String.Format("<{0} value=\"{1}\" />", k, value + "  "));
                    }
                    i++;
                }

                //if (eff.Data.ContainsKey("effSlotType")) { continue; } 
        ...
        private string returnStrings(List<string> tokenList, string separator1, string separator2)
        {
            var tokens = "";
            if (tokenList.Count > 0)
            {
                tokens = separator1 + String.Join(separator2, tokenList.ToArray()) + separator1;
            }
            return tokens;
        }
