...
namespace GomLib.Models
{
    public class Tooltip : GameObject
    {
        ...
        internal GameObject Obj_ { get; set; }
        public GameObject Obj
        {
            get
            {
                if (Obj_ == null && PObj == null)
                {
                    Obj_ = new GameObject().Load(Id, Dom_);
                }
                return Obj_;
            }
            set
    ...
    public static class TooltipHelpers
    {
        ...
        public static XElement ItemInnerHTML(this Item itm)
        {
            ...
            if (itm.WeaponSpec != null)
            {
                ...
                try
                {
                    min = itm.Dom_.data.weaponPerLevel.GetStat(itm.WeaponSpec.Id, level, qual, Stat.MinWeaponDamage);
                    max = itm.Dom_.data.weaponPerLevel.GetStat(itm.WeaponSpec.Id, level, qual, Stat.MaxWeaponDamage);  //change this so items without barrels use level 1 premium numbers
                    techpower = itm.Dom_.data.weaponPerLevel.GetStat(itm.WeaponSpec.Id, level, qual, Stat.TechPowerRating);
                    forcepower = itm.Dom_.data.weaponPerLevel.GetStat(itm.WeaponSpec.Id, level, qual, Stat.ForcePowerRating);
                }
                catch (Exception)
                {
            ...
            else if (isEquipable)
            {
                ...
                if (shield != null)
                {
                    ...
                    try
                    {
                        techpower = itm.Dom_.data.shieldPerLevel.GetShield(itm.ArmorSpec, qual, level, Stat.TechPowerRating);
                        forcepower = itm.Dom_.data.shieldPerLevel.GetShield(itm.ArmorSpec, qual, level, Stat.ForcePowerRating);
                        absorbchance = itm.Dom_.data.shieldPerLevel.GetShield(itm.ArmorSpec, qual, level, Stat.MeleeShieldAbsorb);
                        shieldchance = itm.Dom_.data.shieldPerLevel.GetShield(itm.ArmorSpec, qual, level, Stat.MeleeShieldChance);
                    }
                    catch (Exception)
                    {
                        // string dosomething = ""; //suppress for now, break here to debug
                ...
                if (arm != null)
                {
                    ...
                    else
                    {
                        ...
                        try
                        {
                            ...
                            int armor = itm.Dom_.data.armorPerLevel.GetArmor(arm, level, qual, itm.Slots.Where(x => x != SlotType.Any).First());
            ...
            if (itm != null)
            {
                ...
                if (itm.MissionDescriptionId == 0)
                {
                    ...
                    if (itm.Materials != null)
                    {
                        ...
                        {
                            var mat = (Item)new GameObject().Load(kvp.Key, itm.Dom_);
                            if (mat != null)
                            {
        ...
        public static XElement GetHTML(this Achievement itm)
        {
            ...
            if (itm != null)
            {
                ...
                foreach (var tsk in itm.Tasks)
                {
                    ...
                    if (tskName == "")
                    {
                        var tskSub = itm.Dom_.GetObject(tsk.Id);
                        if (tskSub != null)
                        {
                        ...
                        {
                            var area = itm.Dom_.areaLoader.Load(tsk.Id);
                            if (area.Id != 0)
                            {
                                //area.SortMaps();
