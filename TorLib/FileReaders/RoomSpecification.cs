...
namespace TorLib.FileReaders
{
    public class RoomSpecification
    {
        ...
        private static readonly Regex instanceHeaderParser = new Regex("(\\d+)=(\\d+)");
        private static readonly Regex instanceValueParser = new Regex("\\.([^=]+)=([^\\s]*)");
                                    ...
                                    else if (trimmedLine.StartsWith(".ParentInstance="))
                                    {
                                        ...
                                        if (match.Success)
                                        {
                                            ...
                                            if (parentInstanceId != 0)
                                            {
                                                ...
                                                if (!InstancesByParentId.TryGetValue(parentInstanceId, out List<AssetInstance> parentAssetInstances))
                                                {
